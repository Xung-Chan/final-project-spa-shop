...
public class BillService implements IService<BillResponse, BillRequest> {
	@Autowired
	...
	public BillResponse save(BillRequest object) {
		...
		entity.setServices(new HashSet<>(object.getServices().stream().map(x->serviceRepository.findById(x).get()).toList()));
